    ...
    internal sealed class ImportPipeline
    {
            ...
            IReadOnlyList<string> selectedNodeIds, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                // 1.5. External component masters. The screen tree's INSTANCEs reference
                // master COMPONENTs that frequently live in another page of the file (a
                // "Components" library page). Without those masters in our import set,
                // ComponentExtractionPass logs "external library component; skipping" and
                // the screen-side InstanceConverter falls back to inline conversion — every
                // INSTANCE inlines, no PrefabInstance link. Fetch the missing masters via
                // the same /v1/files/{key}/nodes endpoint and treat them as extra component
                // roots: they go through the Components extraction pass (saved to
                // ComponentOutputPath) but NOT the screens-write pass.
        ...
        // subtracts the COMPONENT / COMPONENT_SET masters that already live in the tree,
        // and fetches the remaining "external" masters from the same Figma file via
        // /v1/files/{key}/nodes. Iterates so transitively-referenced masters get pulled
        // in too (a fetched master may itself contain INSTANCEs of yet other externals).
        // Returned list contains the master trees in their fully-resolved form, ready to
        // feed into the inventory + extraction pass.
            ...
            for (int round = 0; round < maxRounds; round++)
            {
                ...
                foreach (var id in idList)
                {
                    ...
                    if (response.Nodes.TryGetValue(id, out var wrapper) && wrapper?.Document != null)
                    {
                        ...
                        // Walk the new master to discover any nested INSTANCE references it
                        // brings in — those may need fetching next round.
